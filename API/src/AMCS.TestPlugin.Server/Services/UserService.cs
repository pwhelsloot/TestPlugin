namespace AMCS.TestPlugin.Server.Services
{
  ...
  public class UserService : IUserService
  {
    ...
    public string ApplicationCode { get; } = "TST";

    public bool SupportsOfflineMode => false;

    public UserService(ISessionTokenManager sessionTokenManager, ITestPluginConfiguration testPluginConfiguration)
    {
      this.sessionTokenManager = sessionTokenManager;
      tenantId = testPluginConfiguration.TenantId;
    }
    ...
    private IAuthenticationResult AuthenticateByIdentity(string identity, string tenant)
    {
      ...
      if (!string.Equals(tenant, DataServices.Resolve<ITestPluginConfiguration>().TenantId, StringComparison.OrdinalIgnoreCase))
        return new AuthenticationFailure(AuthenticationStatus.InvalidCredentials);
